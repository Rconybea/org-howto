#+title: [23may2026] setting up forgjo runner on a cloud host
# ----------------------------------------------------------------
#+tags: @forgejo @nixpkgs
# ----------------------------------------------------------------
#+description: Setting up forgejo runner on a cloud host
#
# org-publish options
#
...
One advantage is we can setup a host runner so that we share the nix store
between invocations
...
*** Runtime environment for forgejo runner
...
*** Add runner to forgejo instance

Create runner on https://conybeare.us/git/admin/actions/runners.
There's a "create new runner" button.
...
Also turns out this environment applies to systemd starting foregejo.
This doesn't get inherited by CI jobs.
